...
function transformToLis(obj){
  ...
  for(let i=0 ;i<array.length;i++){
    for(let j=0;j<array[1].length-1;j++){
      resArr.push(`<li>${array[i][j]}: ${array[i][j+1]}</li>`);
    }
  }
  return resArr;
...
const divisibleByFiveTwoToThePower = (input) => {
  let array =input.map(item=>item.filter(sitem=>sitem%5===0));
  array=array.map(item=>item.filter(sitem=>typeof(sitem)==='number'));
  let retArr;
  retArr=array.map(item=>item.map(sitem=>Math.pow(2,sitem)));
  // Solution code here...
  return retArr;
};
